...
Streamlit app to align FASTA sequences, compute k‑mer stats, find & highlight motifs, build identity matrices, infer phylogenies (FastTree / IQ-TREE), and auto‑generate a narrative for **general‑audience** or **technical** readers. State is preserved with `st.session_state`, so changing k‑mers, motifs, or story mode doesn’t recompute everything.
...
Python libs (see `requirements.txt` / `environment.yml`): streamlit, numpy, pandas, matplotlib, etc.
...
4. Infer tree (FastTree or IQ-TREE) in a temp dir; render with PhylocanvasGL.  
...
→ Check that the model string is valid (e.g., `GTR+G`), and outputs are written inside the temp dir.
...
Open an issue first for major changes; add screenshots/tests when relevant.
